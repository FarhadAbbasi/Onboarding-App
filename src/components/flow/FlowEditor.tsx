...
export function FlowEditor({ 
  ...
  const [blockEditorMode, setBlockEditorMode] = useState(true) // Always use block editor for proper mobile UI
  ...
  const saveBlocksAndThemeToSupabase = async (blocks: ParsedBlock[], _theme: ParsedTheme, html_content: string) => {
    if (!projectId) {
  ...
  const handleCustomizationsReady = async (customizations: ComponentCustomization[]) => {
    ...
    try {
      ...
      if (customizations.length > 0) {
        ...
        const customizationsToInsert = customizations.map((customization) => ({
          project_id: projectId,
