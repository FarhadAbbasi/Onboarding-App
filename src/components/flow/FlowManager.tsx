...
import { MobileFlowEditor } from './MobileFlowEditor'
...
export function FlowManager({ 
  ...
  const [currentStep, setCurrentStep] = useState<FlowStep>('editing') // Skip planning since we have mobile flow
  const [flowPlan, setFlowPlan] = useState<FlowPlan | null>(null)
  const [editedPages, setEditedPages] = useState<Page[]>([])
  const [useMobileEditor, setUseMobileEditor] = useState(true) // Use mobile editor by default

  const steps = [
    { id: 'planning', label: 'Plan Flow', description: 'AI generates onboarding structure' },
    { id: 'editing', label: 'Edit Content', description: 'Customize mobile UI with live preview' },
    { id: 'publishing', label: 'Publish Flow', description: 'Deploy your onboarding experience' }
  ]

  const handleFlowGenerated = (plan: FlowPlan) => {
    setFlowPlan(plan)
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 'editing':
        // Use MobileFlowEditor for projects with mobile flow data
        if (useMobileEditor) {
          return (
            <MobileFlowEditor
              projectId={projectId}
              ...
              projectUrl={projectUrl}
              onBack={onBack}
              onSave={() => setCurrentStep('publishing')}
            />
          )
        }
        
        // Fallback to old editor if no mobile flow
