...
export function MobilePreview({ blockTree, selectedBlockId, onSelectBlock, onDeleteBlock }: MobilePreviewProps) {
  ...
  const getClassName = (content: string | { text?: string; className?: string }) => {
    return typeof content === 'object' ? content.className || '' : '';
  };

  const parseTestimonial = (content: string | { text?: string; className?: string }) => {
    const contentStr = typeof content === 'string' ? content : content?.text || '';
    try {
      return JSON.parse(contentStr)
    } catch {
      return { text: contentStr, author: '', role: '', company: '' }
    }
  }
                    ...
                    >
                      ...
                      {(() => {
                        switch (block.type) {
                          case 'headline':
                            return (
                              <h1 className={`text-2xl font-bold text-gray-900 text-center mb-4 ${getClassName(block.content)}`} style={style}>
                                {typeof block.content === 'string' ? block.content : block.content?.text || 'Your Headline'}
                              </h1>
                            )
                          case 'subheadline':
                            return (
                              <p className={`text-base text-gray-600 text-center mb-6 max-w-xs mx-auto ${getClassName(block.content)}`} style={style}>
                                {typeof block.content === 'string' ? block.content : block.content?.text || 'Your subheadline goes here'}
                              </p>
                            )
                          case 'feature':
                            return (
                              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                                <div className="flex items-start gap-3">
                                  ...
                                  <p className={`text-gray-700 text-sm ${getClassName(block.content)}`} style={style}>
                                    {typeof block.content === 'string' ? block.content : block.content?.text || 'Feature description'}
                                  </p>
                                </div>
                              </div>
                            )
                          case 'cta':
                            return (
                              <div className="text-center mb-6">
                                <button className={`bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-3 rounded-lg text-base transition-colors shadow-md hover:shadow-lg transform hover:scale-105 ${getClassName(block.content)}`} style={style}>
                                  {typeof block.content === 'string' ? block.content : block.content?.text || 'Get Started'}
                                </button>
                              </div>
                            )
                          case 'testimonial':
                            ...
                            return (
                              <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6">
                                <div className="mb-3">
                                  <p className={`text-gray-700 italic ${getClassName(block.content)}`} style={style}>
