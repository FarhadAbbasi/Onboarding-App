...
// NEW: Pattern-based AI response for our 3-layer architecture
export const AIPatternResponseSchema = z.object({
  success: z.boolean(),
  content: z.object({
    tone: z.enum(['professional', 'playful', 'modern', 'minimal']),
    patterns: z.array(z.object({
      pattern: z.enum(['HeroSplit', 'HeroCentered', 'FeatureGrid3x2', 'FeatureAlternating', 'TestimonialCarousel', 'PricingTiers', 'FAQ', 'ContactForm']),
      props: z.record(z.any()) // Flexible props object
    }))
  }).optional(),
  tokensUsed: z.number(),
...
export type AIPatternResponse = z.infer<typeof AIPatternResponseSchema>
...
export type BlockNode = {
  id: string;
  type: string;
  content: string | { text?: string; className?: string };
  children: string[];
